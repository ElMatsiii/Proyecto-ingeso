window.addEventListener("DOMContentLoaded", () => {
  const container = document.getElementById("detalleContainer") as HTMLElement;
  ...
  function renderAttacks(attacks: any[]): string {
  ...
  async function loadRecommendedCards(excludeId: string) {
  try {
    const res = await fetch("https://api.tcgdex.net/v2/es/cards");
    const data: any[] = await res.json();
    const sample: { id: string; name: string; image?: string }[] = takeRandom(
      data.filter((c: any) => c.id !== excludeId),
      8
    );

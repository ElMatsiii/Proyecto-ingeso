...
window.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchDetailedCards(briefs) {
    ...
    function mapTypeToEn(typeEsOEn) {
        const es2en = {
    ...
    function hydrateTypeOptions(cards) {
        if (!filterType)
            return;
        const first = filterType.options[0];
        filterType.innerHTML = "";
        if (first)
            filterType.appendChild(first);
        const types = new Set();
